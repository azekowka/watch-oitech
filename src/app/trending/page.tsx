...
const WatchHomePage: React.FC = () => {
  ...
  const sidebarItems = [
    { icon: '/images/img_film.svg', label: 'Home', active: false, link: '/' },
    { icon: '/images/img_heart.svg', label: 'Favourites', active: false, link: '/favourites' },
    { icon: '/images/img_trending_up.svg', label: 'Trending', active: true, link: '#' },
    { icon: '/images/img_calendar.svg', label: 'Coming soon', active: false, link: '#' },
    { icon: '/images/img_users.svg', label: 'Community', active: false, link: '#' },
    { icon: '/images/img_search.svg', label: 'Social', active: false, link: '#' },
    { icon: '/images/img_sliders.svg', label: 'Settings', active: false, link: '#' },
    { icon: '/images/img_log_out.svg', label: 'Logout', active: false, link: '#' }
  ];
  ...
  return (
    <div className="flex flex-row justify-start items-center w-full min-h-screen bg-[linear-gradient(358deg,#37302a_0%,#191817_50%,#191817_100%)]">
      ...
      <button 
        className="fixed top-4 left-4 z-50 lg:hidden p-3 bg-[#21201e] rounded-lg shadow-lg" 
      ...
      {/* Sidebar */}
      <aside className={`${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 fixed lg:relative z-40 w-64 lg:w-[18%] h-full bg-[#21201e] shadow-[2px_0px_90px_#6100c266] transition-transform duration-300 ease-in-out`}>
        <div className="flex flex-col justify-start items-center w-full h-full px-6 sm:px-8 lg:px-9 py-8 sm:py-10 lg:py-9 gap-12 sm:gap-14 lg:gap-14">
          {/* Logo */}
          <div className="flex flex-row justify-start items-center w-full">
            ...
            />
            <h1 className="text-lg sm:text-xl lg:text-[22px] font-bold font-poppins leading-8 lg:leading-[33px] tracking-wide uppercase text-global-2 ml-2 sm:ml-3 lg:ml-2">
              WATCH
            </h1>
          </div>
          {/* Navigation Items */}
          <nav className="flex flex-col justify-start items-center w-full gap-6 sm:gap-8 lg:gap-8">
            {sidebarItems.slice(0, 4).map((item, index) => (
              ...
              >
                <Image
                  ...
                  width={24}
                  height={24}
                  className="w-5 h-5 sm:w-6 sm:h-6 lg:w-6 lg:h-6 group-hover:opacity-80 transition-opacity"
                />
                <span className={`text-sm sm:text-base lg:text-base font-poppins leading-6 ml-3 sm:ml-4 lg:ml-3 transition-colors ${
            ...
            {/* Spacer */}
            <div className="h-16 sm:h-20 lg:h-32"></div>
            ...
            {sidebarItems.slice(4).map((item, index) => (
              ...
              >
                <Image
                  ...
                  className="w-5 h-5 sm:w-6 sm:h-6 lg:w-6 lg:h-6 group-hover:opacity-80 transition-opacity"
                />
                <span className="text-sm sm:text-base lg:text-base font-normal font-poppins leading-6 text-global-3 group-hover:text-global-2 ml-3 sm:ml-4 lg:ml-5 transition-colors">
                  {item.label}
                </span>
              </Link>
            ))}
      ...
      {/* Main Content */}
      <main className="flex flex-col justify-start items-start w-full lg:w-[82%] ml-0 lg:ml-8 px-4 sm:px-6 lg:px-8 py-4 sm:py-6 lg:py-10">
        {/* Header */}
        <Header className="w-full mb-12 sm:mb-16 lg:mb-14" />
        {/* Trending Section */}
        <section className="w-full mb-12 sm:mb-16 lg:mb-14">
          <h2 className="text-lg sm:text-xl lg:text-xl font-semibold font-poppins leading-8 lg:leading-[30px] text-global-2 mb-4 sm:mb-6 lg:mb-4">
              ...
              <div key={movie.id} className="relative w-full">
                <div className="relative w-full h-64 sm:h-72 lg:h-[300px] rounded-xl sm:rounded-2xl lg:rounded-[20px] overflow-hidden group">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 bg-[linear-gradient(148deg,#fffffff2_0%,#fffffff2_100%)] rounded-b-xl sm:rounded-b-2xl lg:rounded-b-[20px] p-4 sm:p-5 lg:p-5 shadow-[0px_4px_20px_#888888ff]">
                    <h3 className="text-sm sm:text-base lg:text-base font-semibold font-poppins leading-6 text-global-1 mb-1">
                      {movie.title}
                    </h3>
                    <p className="text-xs sm:text-sm lg:text-sm font-normal font-poppins leading-5 lg:leading-[21px] text-global-1">
        ...
        <section className="flex flex-col lg:flex-row justify-start items-center w-full gap-6 sm:gap-8 lg:gap-6">
          ...
          <div className="flex flex-col justify-start items-center w-full lg:w-1/2 px-0 lg:px-4">
            ...
            <div className="flex flex-col sm:flex-row justify-center items-start sm:items-center w-full mb-4 sm:mb-6 lg:mb-4 gap-2 sm:gap-4">
              <h2 className="text-2xl sm:text-3xl lg:text-[32px] font-semibold font-poppins leading-10 lg:leading-[49px] text-global-2">
                {featuredMovie.title}
              </h2>
              <div className="flex flex-row justify-end items-center gap-2 sm:gap-3 lg:gap-2">
                ...
                />
                <span className="text-base sm:text-lg lg:text-lg font-medium font-poppins leading-7 text-global-2">
            ...
            {/* Description */}
            <p className="text-sm sm:text-base lg:text-base font-normal font-poppins leading-6 text-global-2 w-full mb-6 sm:mb-8 lg:mb-7">
            ...
            <div className="flex flex-col sm:flex-row justify-start items-center w-full gap-3 sm:gap-4 lg:gap-4">
              <Button
                ...
                variant="primary"
                size="md"
                className="w-full sm:w-auto bg-button-1 text-button-1 hover:bg-purple-700 px-6 sm:px-8 lg:px-6 py-3 sm:py-4 lg:py-3 rounded-lg sm:rounded-xl lg:rounded-[14px] font-medium"
              >
                Watch now
              </Button>
              <button
